...
### Fixed
- **Publishing** - Internal modules (`jafar-parser-core`, `jafar-parser-codegen`) no longer leak into published POMs
  - `jafar-parser` and `jafar-tools` now publish shadow JARs with internal modules embedded
  - External transitive dependencies (slf4j, asm) remain in POMs for consumers
