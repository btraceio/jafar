...
public final class HeapDumpParser {
  ...
  /**
   * Resolves AUTO parsing mode to IN_MEMORY or INDEXED based on heap dump file size. Files larger
   * than 2GB use indexed mode, smaller files use in-memory mode.
   */
  private static ParserOptions resolveParsingMode(Path path, ParserOptions options)
      throws IOException {
    ...
    org.slf4j.LoggerFactory.getLogger(HeapDumpParser.class)
        .info(
  ...
  public enum ParsingMode {
    /**
     * Automatically choose between IN_MEMORY and INDEXED based on heap dump size. Files &gt;2GB use
     * indexed mode, smaller files use in-memory mode.
     */
    AUTO,

    /**
     * In-memory parsing mode. Fast for small heaps (&lt;10M objects, &lt;2GB file size). All
     * objects stored in memory during parsing.
     */
    IN_MEMORY,

    /**
     * Index-based parsing mode. Scalable for large heaps (up to 114M+ objects). Uses disk-based
     * indexes and memory-mapped I/O.
     */
    INDEXED
  }
  ...
  public record ParserOptions(
    ...
    /**
     * Default object cache size for indexed mode (100,000 objects = ~3-4MB). Provides good balance
     * between memory usage and query performance.
     */
    public static final int DEFAULT_OBJECT_CACHE_SIZE = 100_000;

    /**
     * Default options: auto-detect parsing mode, no dominators, no inbound refs. Files &gt;2GB
     * automatically use indexed mode for scalability.
     */
    public static final ParserOptions DEFAULT =
        new ParserOptions(false, true, false, ParsingMode.AUTO, DEFAULT_OBJECT_CACHE_SIZE);

    /** Options for full analysis including dominator computation (auto-detect mode). */
    public static final ParserOptions FULL_ANALYSIS =
        new ParserOptions(true, true, true, ParsingMode.AUTO, DEFAULT_OBJECT_CACHE_SIZE);

    /** Options for minimal memory usage (in-memory mode). */
    public static final ParserOptions MINIMAL =
        new ParserOptions(false, false, false, ParsingMode.IN_MEMORY, DEFAULT_OBJECT_CACHE_SIZE);

    /**
     * Options for index-based parsing (for large heaps &gt;10M objects). Uses disk-based indexes
     * instead of in-memory maps, enabling analysis of heaps up to 114M+ objects with &lt;4GB heap.
     */
    public static final ParserOptions INDEXED =
        new ParserOptions(false, true, false, ParsingMode.INDEXED, DEFAULT_OBJECT_CACHE_SIZE);

    /**
     * Options for in-memory parsing (for small heaps &lt;10M objects). Fast but memory-intensive,
     * all objects stored in memory.
     */
    public static final ParserOptions IN_MEMORY =
        new ParserOptions(false, true, false, ParsingMode.IN_MEMORY, DEFAULT_OBJECT_CACHE_SIZE);
    ...
    public static class Builder {
      ...
      public ParserOptions build() {
        return new ParserOptions(
            computeDominators, indexStrings, trackInboundRefs, parsingMode, objectCacheSize);
      }
    }
  }
