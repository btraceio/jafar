 ...
 *
 * <p>This reader provides fast access to inbound reference counts using memory-mapped I/O. Inbound
 * counts are used for approximate retained size computation.
 *
...
public final class InboundCountReader implements AutoCloseable {
   ...
   *
   * <p>Uses direct offset calculation for O(1) access: offset = HEADER_SIZE + (id32 × ENTRY_SIZE)
   *
  ...
  public int getInboundCount(int objectId32) {
    ...
    // Calculate entry offset with long arithmetic to avoid int overflow for >268M objects
    long offset =
        (long) IndexFormat.HEADER_SIZE
            + (long) objectId32 * IndexFormat.INBOUND_ENTRY_SIZE
