...
class HprofSpecificationComplianceTest {
  ...
  void testA3_AllGcRoots_InMemory() throws Exception {
    ...
    for (var root : heap.getGcRoots()) {
      ...
      assertTrue(
          obj.isPresent(), "GC root should reference existing object: " + root.getObjectId());
    }
  }
  ...
  void testA5_InstanceFields_InMemory() throws Exception {
    ...
    long[] refIds = holder.getOutboundReferences().mapToLong(HeapObject::getId).sorted().toArray();
    assertArrayEquals(
  ...
  void testB3_CircularReferences_InMemory() throws Exception {
    ...
    long[] refs1 = selfRef.getOutboundReferences().mapToLong(HeapObject::getId).toArray();
        ...
        .forEach(
            inMemObj -> {
              ...
              assertTrue(
                  ...
                  testName + ": Object " + inMemObj.getId() + " should exist in indexed mode");
