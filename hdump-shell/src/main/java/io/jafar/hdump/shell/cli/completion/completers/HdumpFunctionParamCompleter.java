...
public final class HdumpFunctionParamCompleter implements ContextCompleter<HdumpMetadataService> {
  ...
  public void complete(
    ...
    if (DEBUG) {
      System.err.println(
          "[FunctionParamCompleter] rootType="
              + rootType
              + ", func="
              + functionName
              + ", partial='"
    ...
    if ("checkLeaks".equals(functionName)) {
      ...
      if ("detector=".startsWith(partial) || partial.isEmpty()) {
        candidates.add(
            candidateNoSpace(prefix + "detector=", "detector=", "built-in detector name"));
      }
      if ("filter=".startsWith(partial) || partial.isEmpty()) {
        candidates.add(
            candidateNoSpace(prefix + "filter=", "filter=", "custom filter query reference"));
      }
      if ("threshold=".startsWith(partial) || partial.isEmpty()) {
        candidates.add(
            candidateNoSpace(prefix + "threshold=", "threshold=", "minimum count threshold"));
      }
    ...
    if ("groupBy".equals(functionName) && ctx.parameterIndex() > 0) {
      ...
      if ("agg=".startsWith(partial) || partial.isEmpty()) {
        candidates.add(
            candidateNoSpace(prefix + "agg=", "agg=", "aggregation type (count/sum/avg/min/max)"));
      }
      if ("value=".startsWith(partial) || partial.isEmpty()) {
        candidates.add(
            candidateNoSpace(prefix + "value=", "value=", "value expression to aggregate"));
      }
  ...
  /**
   * Detects if we're inside a value expression context. Returns the context with the expression
   * being typed and the partial identifier.
   */
  ...
  /**
   * Parse a value expression to find the partial identifier being typed. Handles expressions like
   * "instanceSize * inst" -> partial="inst", exprPrefix="instanceSize * "
   */
  ...
  /**
   * Complete field names inside a value expression. The prefix calculation must account for what
   * JLine considers the current "word".
   */
  private void completeValueExprFields(
      ValueExprContext valueCtx,
      List<String> fields,
      String jlineWord,
    ...
    //
    // Example: If user typed "value=inst" and jlineWord is "inst", completion should be
    // "instanceSize"
    // Example: If user typed "value=" and jlineWord is "", completion should be "instanceSize"
    // Example: If user typed "value=" and jlineWord is "value=", completion should be
    // "value=instanceSize"
    ...
    if (DEBUG) {
      System.err.println(
          "[FunctionParamCompleter] ValueExpr context: identPartial='"
              + identPartial
              + "', jlineWord='"
              + jlineWord
              + "', completionPrefix='"
