...
class HdumpShellCompleterTest {
  ...
  void setUp() {
    ...
    SessionManager sessions = new SessionManager((path, ctx) -> new MockHeapSession(path), null);
    try {
  ...
  void testGroupByFunctionCompletion() {
    ...
    assertTrue(
        values.contains("groupBy(instanceCount"), "Should contain 'groupBy(instanceCount' field");
  ...
  void testSelectFunctionWithObjectsRoot() {
    ...
    assertTrue(
        values.contains("select(retainedSize"), "Should contain 'select(retainedSize' field");
  }
  ...
  /**
   * Create a ParsedLine that simulates JLine's DefaultParser behavior. JLine treats whitespace as
   * word separators and includes an empty word when cursor is after a separator.
   */
  private ParsedLine createParsedLine(String line, int inputCursor) {
    ...
    return new ParsedLine() {
      ...
      @Override
      public String word() {
        return finalCurrentWord;
      }

      @Override
      public int wordCursor() {
        return finalCurrentWord.length();
      }
