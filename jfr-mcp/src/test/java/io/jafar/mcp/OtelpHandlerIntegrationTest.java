...
import io.jafar.otelp.shell.MinimalOtelpBuilder;
...
/**
 * Integration tests for otelp_* MCP handlers against a synthetic OTLP profile. Tests the full
 * handler path: open → query/analysis → close.
 */
class OtelpHandlerIntegrationTest {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // otelp_open / otelp_close
  // ─────────────────────────────────────────────────────────────────────────────

  @Test
  void openReturnsSessionInfo() throws Exception {
    var result = invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
  ...
  void openWithAliasThenCloseByAlias() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString(), "alias", "myprofile"));
    var closeResult = invoke("handleOtelpClose", Map.of("sessionId", "myprofile"));
    assertSuccess(closeResult);
  }

  @Test
  void openDuplicateAliasReturnsError() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString(), "alias", "dup"));
    var result = invoke("handleOtelpOpen", Map.of("path", profilePath.toString(), "alias", "dup"));
  ...
  void queryCountReturnsSampleCount() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpQuery", Map.of("query", "samples | count"));
    assertSuccess(result);
  ...
  void queryGroupByReturnsLeafFunctions() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result =
        invoke("handleOtelpQuery", Map.of("query", "samples | groupBy(stackTrace/0/name)"));
    assertSuccess(result);
    String content = result.content().get(0).toString();
  ...
  void queryParseErrorReturnsError() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpQuery", Map.of("query", "$$invalid$$"));
    assertTrue(result.isError());
  }
  ...
  void queryWithLimitTruncates() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpQuery", Map.of("query", "samples | top(4, cpu)", "limit", 1));
    assertSuccess(result);
  ...
  void summaryReturnsSampleTypeInfo() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpSummary", Map.of());
    assertSuccess(result);
    String content = result.content().get(0).toString();
  ...
  void flamegraphReturnsStackRows() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpFlamegraph", Map.of());
    assertSuccess(result);
    String content = result.content().get(0).toString();
  ...
  void flamegraphRejectsFilterWithClosingBracket() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invoke("handleOtelpFlamegraph", Map.of("filter", "cpu > 0] | top(5)"));
  ...
  void useReturnsStructuredReport() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invokeWithExchange("handleOtelpUse", Map.of());
    assertSuccess(result);
    String content = result.content().get(0).toString();
  ...
  void useThreadsReturnsDistribution() throws Exception {
    invoke("handleOtelpOpen", Map.of("path", profilePath.toString()));
    var result = invokeWithExchange("handleOtelpUse", Map.of("resources", List.of("threads")));
    assertSuccess(result);
    String content = result.content().get(0).toString();
