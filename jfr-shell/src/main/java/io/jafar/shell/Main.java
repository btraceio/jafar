...
public class Main implements Callable<Integer> {
    @CommandLine.Option(names = {"-f", "--file"}, description = "JFR file to open immediately (interactive mode)")
    private String jfrFile;

    @CommandLine.Option(names = {"-q", "--quiet"}, description = "Suppress banner (interactive mode)")
    ...
    abstract static class NonInteractiveCommand implements Callable<Integer> {
        ...
        protected Integer executeNonInteractive(String command) {
            ...
            try {
                ...
                CommandDispatcher dispatcher = new CommandDispatcher(sessions, new CommandDispatcher.IO() {
                    @Override
    ...
    @CommandLine.Command(
        name = "show",
        description = "Execute a JfrPath query and display results"
    )
    static class ShowCommand extends NonInteractiveCommand {
        @CommandLine.Parameters(index = "1", description = "JfrPath expression (e.g., \"events/jdk.FileRead | count()\")")
        private String expression;

        @CommandLine.Option(names = {"--limit", "-l"}, description = "Limit number of results")
        private Integer limit;

        @CommandLine.Option(names = {"--format", "-f"}, description = "Output format: table (default), json")
        private String format = "table";

        @CommandLine.Option(names = {"--list-match"}, description = "List matching mode: any, all, none")
    ...
    @CommandLine.Command(
        name = "metadata",
        description = "List event types and metadata"
    )
    static class MetadataCommand extends NonInteractiveCommand {
        @CommandLine.Option(names = {"--search", "-s"}, description = "Search pattern")
        private String search;

        @CommandLine.Option(names = {"--regex", "-r"}, description = "Use regex for search")
        private boolean regex;

        @CommandLine.Option(names = {"--events-only", "-e"}, description = "Show only event types")
        private boolean eventsOnly;

        @CommandLine.Option(names = {"--summary"}, description = "Show summary only")
    ...
    @CommandLine.Command(
        name = "chunks",
        description = "List chunk information"
    )
    ...
    static class CpCommand extends NonInteractiveCommand {
        @CommandLine.Option(names = {"--type", "-t"}, description = "Constant pool type name")
        private String type;
