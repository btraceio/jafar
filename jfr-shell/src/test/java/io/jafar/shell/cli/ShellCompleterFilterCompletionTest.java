...
class ShellCompleterFilterCompletionTest {
    ...
    void suggestsEventAttributesWhenFilterOpenedAfterType() throws Exception {
        ...
        boolean hasStackTrace = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[stackTrace"));
        boolean hasSampledThread = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[sampledThread"));
        assertTrue(hasStackTrace || hasSampledThread, "Expected field suggestions inside event filter after type");
    }
    ...
    void suggestsEventAttributesWhenFilterOpenedAfterNestedPath() throws Exception {
        ...
        assertTrue(hasFrames || hasTruncated, "Expected field suggestions inside event filter at nested path");
    }
    ...
    void suggestsCpAttributesWhenFilterOpenedAfterType() throws Exception {
        ...
        boolean hasString = cands.stream().anyMatch(c -> c.value().equals("cp/jdk.types.Symbol[string"));
