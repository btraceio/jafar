...
class ShellCompleterFilterFunctionsTest {
  ...
  void suggestsFunctionTemplatesInEventFilter() throws Exception {
    ...
    boolean hasContains =
        cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[contains("));
    boolean hasExists =
        cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[exists("));
  ...
  void suggestsFunctionTemplatesInMetadataFilter() throws Exception {
    ...
    boolean hasContains =
        cands.stream().anyMatch(c -> c.value().equals("metadata/jdk.ExecutionSample[contains("));
    boolean hasExists =
        cands.stream().anyMatch(c -> c.value().equals("metadata/jdk.ExecutionSample[exists("));
    assertTrue(
        hasContains || hasExists, "Expected function template suggestions in metadata filter");
  }
}
