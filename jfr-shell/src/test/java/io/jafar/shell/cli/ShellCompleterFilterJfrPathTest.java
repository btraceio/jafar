...
class ShellCompleterFilterJfrPathTest {
    ...
    void suggestsNestedPathsInsideEventFilter() throws Exception {
        ...
        boolean hasFrames = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[stackTrace/frames"));
        boolean hasTruncated = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[stackTrace/truncated"));
    ...
    void suggestsListPrefixAndFieldsInsideEventFilter() throws Exception {
        ...
        boolean hasAnyStackTrace = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[any:stackTrace"));
        boolean hasAnySampledThread = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[any:sampledThread"));
        assertTrue(hasAnyStackTrace || hasAnySampledThread, "Expected list-aware field suggestions after any:");
    }
    ...
    void suggestsNestedPathsAfterListPrefixInsideEventFilter() throws Exception {
        ...
        var pl = new ShellCompleterTest.SimpleParsedLine("show events/jdk.ExecutionSample[any:stackTrace/");
        completer.complete(null, pl, cands);

        boolean hasFrames = cands.stream().anyMatch(c -> c.value().equals("events/jdk.ExecutionSample[any:stackTrace/frames"));
