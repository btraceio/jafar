...
class ShowConstantPoolEntriesTest {
    ...
    void listsSymbolConstantPoolEntries() throws Exception {
        ...
        assertTrue(summary.contains("jdk.types.Symbol"), "Expected 'jdk.types.Symbol' in CP summary.\n" + summary);

        // Entries for a specific CP type
        io.out.setLength(0);
        dispatcher.dispatch("show cp/jdk.types.Symbol");
        String rows = io.text();
        assertFalse(rows.contains("(no rows)"), "Unexpected (no rows) for cp/jdk.types.Symbol.\n" + rows);
        assertTrue(rows.contains("| id") || rows.contains("| value") || rows.contains("| symbol") || rows.contains("| string"),
