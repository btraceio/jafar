...
class ShowConstantPoolFilterTest {
    ...
    void filtersCpEntriesByStringEquality() throws Exception {
        ...
        List<Map<String, Object>> all = evaluator.evaluate(sessions.getCurrent().get().session, JfrPathParser.parse("cp/jdk.types.Symbol"));
        ...
        for (Map<String, Object> r : all) {
            ...
            if (v instanceof String s && !s.isEmpty()) { match = s; break; }
        }
        assertNotNull(match, "Expected a non-empty 'string' in symbol pool");

        // Now filter by that exact string
        String expr = "cp/jdk.types.Symbol[string=\"" + match.replace("\\", "\\\\").replace("\"", "\\\"") + "\"]";
        List<Map<String, Object>> filtered = evaluator.evaluate(sessions.getCurrent().get().session, JfrPathParser.parse(expr));
