 ...
 * OtelpProfile.ProfilesData}.
 */
public final class OtelpSession implements Session {

  private static final Logger LOG = LoggerFactory.getLogger(OtelpSession.class);

  private final Path path;
  private final OtelpProfile.ProfilesData data;
  private volatile boolean closed = false;

  private OtelpSession(Path path, OtelpProfile.ProfilesData data) {
    this.path = path;
    this.data = data;
  }
   ...
   */
  public static OtelpSession open(Path path) throws IOException {
    LOG.info("Opening OTLP profiles: {}", path);
    OtelpProfile.ProfilesData data = OtelpReader.read(path);
    ...
    return new OtelpSession(path, data);
  }

  /** Returns the parsed profiles data for direct access by the evaluator. */
  public OtelpProfile.ProfilesData getData() {
    return data;
  }
  ...
  public Map<String, Object> getStatistics() {
    ...
    if (!data.profiles().isEmpty()) {
      OtelpProfile.Profile first = data.profiles().get(0);
      OtelpProfile.ValueType st = first.sampleType();
