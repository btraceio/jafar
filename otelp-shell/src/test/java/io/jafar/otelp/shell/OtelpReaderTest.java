...
/** Tests for {@link OtelpReader}: round-trip encoding and parsing of synthetic profiles. */
class OtelpReaderTest {
  ...
  void roundTripBasicProfile() throws Exception {
    ...
    OtelpProfile.ProfilesData data = OtelpReader.read(file);

    // profiles
    assertEquals(1, data.profiles().size());
    OtelpProfile.Profile profile = data.profiles().get(0);
    ...
    OtelpProfile.Sample sample = profile.samples().get(0);
