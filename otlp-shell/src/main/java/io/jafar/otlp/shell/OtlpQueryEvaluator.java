...
/**
 * {@link QueryEvaluator} implementation for the OtlpPath query language. Bridges the shell
 * infrastructure with the OTLP profiling query system.
 */
public final class OtlpQueryEvaluator implements QueryEvaluator {
  ...
  public Object parse(String queryString) throws QueryParseException {
    try {
      return OtlpPathParser.parse(queryString);
  ...
  public Object evaluate(Session session, Object query) throws Exception {
    OtlpSession otlpSession = requireOtlpSession(session);
    SamplesPath.Query otlpQuery = toQuery(query);
    return OtlpPathEvaluator.evaluate(otlpSession, otlpQuery);
  }
  ...
  public VariableStore.LazyValue createLazyValue(
    ...
    OtlpSession otlpSession = requireOtlpSession(session);
    SamplesPath.Query otlpQuery = toQuery(query);
    return new LazyOtlpValue(otlpSession, otlpQuery, queryString);
  }

  private OtlpSession requireOtlpSession(Session session) {
    if (!(session instanceof OtlpSession os)) {
      throw new IllegalArgumentException(
          "OtlpQueryEvaluator requires an OtlpSession, got: " + session.getClass().getSimpleName());
    }
    return os;
  }
  ...
  /** Lazy value that evaluates an OtlpPath query on demand. */
  private static final class LazyOtlpValue implements VariableStore.LazyValue {
    private final OtlpSession session;
    private final SamplesPath.Query query;
    private final String queryString;
    private List<Map<String, Object>> cached;
    private boolean evaluated;

    LazyOtlpValue(OtlpSession session, SamplesPath.Query query, String queryString) {
    ...
    public synchronized Object get() throws Exception {
      if (!evaluated) {
        cached = OtlpPathEvaluator.evaluate(session, query);
        evaluated = true;
      }
      return cached;
    }
