 ...
 *
 * <p>- Transparently unwraps {@link ComplexType} to its {@code Map<String, Object>} value -
 * Transparently unwraps {@link ArrayType} to its backing Java array when returned - Supports nested
 * navigation using a path of keys/indices
 */
public final class Values {
  private Values() {}

  /**
   * Navigate a nested value using a sequence of keys/indices. Each path element may be a {@link
   * String} key (for maps) or an {@link Integer} index (for arrays).
   *
   * <p>- {@link ComplexType} nodes are unwrapped to {@code Map<String, Object>} automatically. -
   * {@link ArrayType} nodes are unwrapped to the backing array automatically when returned or
   * indexed.
   *
  ...
  /**
   * Typed extraction using {@link #get(Map, Object...)}. Returns an empty optional if the resolved
   * value is null or not of the requested type.
   *
  ...
  /**
   * Shallowly resolve a map: unwraps {@link ComplexType} to maps and {@link ArrayType} to arrays at
   * the top level only.
   *
  ...
  /**
   * Deeply resolve a map: recursively unwraps {@link ComplexType}; unwraps {@link ArrayType} to
   * arrays; recursively resolves elements of {@code Object[]} arrays. Primitive arrays are left
   * as-is.
   *
   * @param root the root map to resolve
  ...
  /**
   * If the value is an {@link ArrayType}, return its backing array; otherwise return the value
   * unchanged.
   */
