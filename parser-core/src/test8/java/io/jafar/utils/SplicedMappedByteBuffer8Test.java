...
/**
 * Verifies Java 8 implementation behavior of SplicedMappedByteBuffer, especially paths where
 * absolute bulk get is not available and logic falls back to emulation via position juggling.
 */
public class SplicedMappedByteBuffer8Test {
  ...
  void testPositionAndRemaining() {
    ...
    assertThrows(BufferOverflowException.class, () -> instance.position(FILE_SIZE + 1));
  }

  @Test
  void testSliceAndSubslice() {
    // Order defaults to native; isNativeOrder reflects the original mapping order
    ...
    // Changing order does not change the isNativeOrder flag
    ...
    sliced1.position(5);
    // ensure independence from parent
    assertEquals(0, instance.position());
    ...
    assertThrows(BufferOverflowException.class, () -> instance.slice(3 * SLICE_SIZE, FILE_SIZE));

    CustomByteBuffer s3 = instance.slice(SLICE_SIZE - 1, 2 * SLICE_SIZE);
    assertEquals(1, s3.getShort());
    CustomByteBuffer s4 = s3.slice(SLICE_SIZE - 1, SLICE_SIZE);
    assertEquals(2, s4.getInt());
  }

  @Test
  void testSimpleGetsAndAcrossSplices() {
    ...
    byte[] expectedBulk = new byte[2 * SLICE_SIZE + 17];
    Arrays.fill(expectedBulk, (byte) 8);
    instance.position(SLICE_SIZE * 6);
    byte[] data = new byte[expectedBulk.length];
    instance.get(data, 0, data.length);
    assertArrayEquals(expectedBulk, data);
  }
}
