...
/**
 * Analyzes the input line and cursor position to determine the completion context for pprof
 * queries.
 *
 * <p>pprof has a single root type ({@code samples}), so there is no type-pattern sub-path. The
 * recognized contexts are: COMMAND, ROOT, FILTER_FIELD, FILTER_OPERATOR, FILTER_LOGICAL,
 * PIPELINE_OPERATOR, and FUNCTION_PARAM.
 */
public final class PprofCompletionContextAnalyzer {
